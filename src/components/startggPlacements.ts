...
import { getStartggPlacements, type SharedPlacements, type StartggPlacement, type StartggPlacementsResult } from '../services/db';
...
export function bindPlacementLogos(host: ParentNode): void {
  qsa<HTMLImageElement>('[data-startgg-logo]', host).forEach((img) => img.addEventListener('error', () => img.remove(), { once: true }));
}

/**
 * Ergebnisse eines anderen Mitglieds im Spielerprofil. Nur lesen: kein Aktualisieren,
 * gezeigt wird der Stand, den der Besitzer zuletzt geladen hat.
 */
export function sharedPlacementsMarkup(shared: SharedPlacements, owner: string): Markup {
  const { placements } = shared;
  return html`${placements.length
      ? html`${summary(placements)}<ol class="placements__list" role="list">${placements.map(row)}</ol>`
      : html`<div class="empty placements__empty"><p>${t('player.noTournaments', { name: owner })}</p></div>`}
    <p class="placements__foot">${t('player.startggAsOf', { time: relative(shared.fetchedAt), name: owner })}</p>`;
}
