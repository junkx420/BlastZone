...
export function tiersPage(): PageView {
  return {
    title: `${t('tiers.title')} | Blastzone`,
    markup: html`<div class="page tiers-page">
      <header class="container page-head">
        <h1 data-reveal="wipe">${t('tiers.title')}</h1>
        <p>${t('tiers.source', { source: TIER_SOURCE.name, date: TIER_SOURCE.published })} ${TIER_SOURCE.note}</p>
        <a class="link" href="${TIER_SOURCE.url}" target="_blank" rel="noopener">${t('tiers.table')}${ICONS.external}</a>
      </header>
              ...
              ${placements.map((p, i) => {
                ...
                return html`<li class="tierchip" style="${accentVars(f.colors)}" data-reveal data-reveal-delay="${Math.min(i * 0.03, 0.3).toFixed(2)}">
                  <a class="tierchip__link" href="${link(`/fighter/${f.slug}`)}">
                    ...
                    <span class="tierchip__rank"><span aria-hidden="true">#</span><span class="vh">${t('tiers.rankVh')}</span>${p.rank}</span>
                    <span class="tierchip__name">${f.name}</span>
                    <span class="tierchip__score"><span class="vh">${t('tiers.scoreVh')}</span>${score(p.score)}</span>
                  </a>
                </li>`;
              })}
            </ol>
