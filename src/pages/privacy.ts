 ...
 * Neue Funktionen mit Personenbezug (etwa eine Pinnwand) brauchen hier einen
 * eigenen Absatz, bevor sie live gehen.
 *
 * Zwei Fassungen: Deutsch ist maßgeblich, Englisch eine Übersetzung mit Hinweis
 * und Link auf die deutsche (privacy-en.ts, nachgeladen). Jede inhaltliche
 * Änderung in beiden Fassungen vornehmen, sonst behauptet eine Sprache etwas
 * anderes als der Code tut.
 */
export function privacyPage(): PageView {
  if (lang !== 'en') return { title: 'Datenschutzerklärung | Blastzone', markup: germanMarkup() };
  return {
    title: 'Privacy Policy | Blastzone',
    markup: html`<div class="page legal" data-privacy>
      <div class="container page-head"><p class="profile-status" role="status">${t('state.loading')}</p></div>
    </div>`,
    mount(root) {
      const host = qs<HTMLElement>('[data-privacy]', root)!;
      let alive = true;
      const load = (): void => {
        import('./privacy-en').then(
          ({ englishSections }) => {
            if (alive) mount(host, englishSections());
          },
          () => {
            if (!alive) return;
            mount(host, html`<div class="container page-head">${errorState(t('api.failed'), LOAD_FAILED_TEXT)}</div>`);
  ...
  return html`<div class="page legal">
      ...
      <section class="container legal__body">
        ...
        <p>
          ...
          Hash, nie im Klartext. Dazu kommt, was du selbst anlegst: deinen Main-Fighter samt gewähltem Skin, das
          Farbschema, gespeicherte Combos und Kommentare, jeweils mit Zeitpunkt. Rechtsgrundlage ist Art. 6 Abs. 1
          lit. b DSGVO, die Bereitstellung des Kontos, das du anlegst.
        </p>
        <p>
          <strong>Kommentare sind öffentlich.</strong> Jeder Besucher sieht sie zusammen mit deinem Benutzernamen,
          deinem Main-Fighter samt Skin und dem Zeitpunkt. E-Mail-Adresse und gespeicherte Combos sieht niemand
          außer dir.
        </p>
        <p>
          ...
          jeweils mit dem gewählten Skin, dem Monat deiner Registrierung, der Anzahl deiner Kommentare und deinen
          letzten Kommentaren. E-Mail-Adresse und gespeicherte Combos erscheinen dort nicht, deine
          start.gg-Ergebnisse nur, wenn du sie selbst freigibst.
        </p>
        <p>
        ...
        <p>
          Die Verknüpfung und die zwischengespeicherten Ergebnisse sieht zunächst nur du. Sie sind nicht Teil deines
        ...
        <p>
          <strong>Deine Turnierergebnisse zeigen wir anderen nur, wenn du das selbst einschaltest.</strong> Im Profil
          gibt es dafür den Schalter „start.gg-Ergebnisse im Spielerprofil zeigen“; er steht anfangs aus. Ist er an
          und ist deine Verknüpfung per start.gg-Anmeldung bestätigt, sehen angemeldete Mitglieder auf deinem
          Spielerprofil deinen dort angezeigten Spielernamen, die Adresse deines start.gg-Profils und die Liste der
          Ergebnisse in dem Stand, den du zuletzt geladen hast. Ihr Aufruf fragt start.gg nicht an. Schaltest du den
          Schalter aus, ist die Liste für andere sofort wieder weg.
        </p>

        <h2>Nachrichten an andere Mitglieder</h2>
        <p>
          Angemeldete Mitglieder können einander Nachrichten schreiben. Gespeichert werden der Text, wer ihn
          geschrieben hat, an wen er ging, der Zeitpunkt und, sobald die Empfängerin oder der Empfänger den Verlauf
          öffnet, der Zeitpunkt des Lesens. Die Daten liegen wie das übrige Konto bei Supabase in Frankfurt am Main.
          Rechtsgrundlage ist Art. 6 Abs. 1 lit. b DSGVO, die Community-Funktionen des Kontos.
        </p>
        <p>
          Lesen können eine Unterhaltung nur die beiden Beteiligten; die Datenbank lässt niemand anderen an diese
          Zeilen. <strong>Eine Ende-zu-Ende-Verschlüsselung ist das nicht:</strong> Als Betreiber hätten wir über die
          Verwaltung der Datenbank technisch Zugriff auf die gespeicherten Texte. Wir lesen dort nicht mit und täten
          es nur, wenn wir rechtlich dazu verpflichtet sind oder ein gemeldeter Missbrauch anders nicht zu klären
          ist. Schreib nichts, was du einem Betreiber nicht anvertrauen würdest.
        </p>
        <p>
          <strong>Du bestimmst, wer dir schreiben darf.</strong> Im Profil kannst du Nachrichten ganz abschalten;
          dann kann dir niemand mehr neue schicken. Einzelne Mitglieder kannst du blockieren: Danach könnt ihr
          einander nicht mehr schreiben, und eure Unterhaltung ist für dich ausgeblendet. Gespeichert wird dafür,
          wen du blockiert hast, mit Zeitpunkt. Die blockierte Person erfährt davon nichts. Rechtsgrundlage ist
          Art. 6 Abs. 1 lit. f DSGVO, das berechtigte Interesse an einem Schutz vor unerwünschtem Kontakt.
        </p>
        <p>
          Gegen Spam begrenzt die Datenbank, wie viele Nachrichten ein Konto pro Minute und Tag verschicken kann und
          an wie viele verschiedene Empfänger am Tag. Nachrichten lassen sich nicht einzeln löschen. Löschst du dein
        ...
        <h2>Stand</h2>
        <p>Diese Erklärung gilt seit dem 24. September 2026.</p>
